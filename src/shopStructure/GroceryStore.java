...
import implementedDatastructures.Dictionary;
import implementedDatastructures.Queue;
...
//		this.groceryStore = name;
	...
	public int addProduct(String name, float price, int barcodeId, int count, String department) {
		if(myDepartments.find(department) == department)
		{
			((Allproducts)findProductByBarcode(getBarcodeId(barcodeId)).addQuantity(count++);
		}
...
//		Allproducts.addProduct(department, name, price, barcodeId, count);



//		//float currentQty = ((Product)getProductByBarcode(barcodeId)).getQuantity();
//		if(myProducts.contains(barcodeId)) {
//			((Allproducts)findProductByBarcode(get.BarCodeId(barcodeId))).addQuantity(count);
////			(Allproducts)findFreshProductByBarcode(getBarcodeId(barcode))
////		if product exist it will increase the quantity
//		}
//		else {
//			Product newProduct = new Product(department,name,price,barcodeId,count);
//			myProducts.add(barcodeId, newProduct);                                      //if product not present this will add as new product
...
//	}
	...
	private int getBarcodeId(int barcodeId) {
		return barcodeId;
	}
	 ...
	 * @param barcode - is the bar-code number of the new fresh product (identifying only the type)
	 * @param amount - is the amount in kilograms the new fresh product in the store
	 */



//	public void addFreshProduct(String name, float pricePerKg, int barcode, float amount) {
//		if(myFreshProducts.contains(getName)) {
//			//float currentQty = ((Product)getFreshProductByBarcode(barcode)).getQuantity();
//			(Allproducts)findFreshProductByBarcode(getBarcodeId(barcode)). addQuantity(amount);  //if fresh product exist it will increase the quantity
//		}
//		else {
//			FreshProducts newFresh = new FreshProducts(name,pricePerKg,barcode,amount);
...
//	}
	...
	public void addToBasket(int barcodeId, int count, int customerId) {
		...
		if (!(count <= ((Product) findProductByBarcode(getBarcodeId(barcodeId)).getQuantity() {
	...
	public void addFreshToBasket(int barcodeId,int customerId,float amount) {
		...
		findClientById(customerId).getBasket().addFreshProduct((FreshProducts)findFreshProductByBarcode(getBarcodeId(barcodeId)), amount); // here we are adding fresh product to basket
